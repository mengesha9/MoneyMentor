...
class SyncConfig:
    ...
    startup_delay_seconds: int = int(os.getenv('SYNC_STARTUP_DELAY_SECONDS', '1800'))  # 30 minutes initial delay
...
class BackgroundSyncService:
    ...
    async def start_background_sync(self):
        ...
        self.sync_stats['start_time'] = self.service_start_time
        self.sync_stats['health_status'] = 'starting'
        logger.info("🚀 Starting enhanced background sync service...")
        logger.info(f"📊 Sync interval: {self.config.interval_seconds} seconds (30 minutes)")
        logger.info(f"⏳ Startup delay: {self.config.startup_delay_seconds} seconds (30 minutes)")
        ...
        logger.info(f"⏰ First sync will start after {self.config.startup_delay_seconds} seconds (30 minutes)")
        logger.info(f"⏰ Subsequent syncs every {self.config.interval_seconds} seconds (30 minutes)")
    ...
    async def _initialize_sheets_service(self):
        ...
        try:
            ...
            # Initialize comprehensive sync service
            ...
            try:
                ...
                await self.comprehensive_sync_service.initialize()
        ...
        while self.is_running:
            try:
                ...
                if self.service_start_time:
                    time_since_startup = (datetime.utcnow() - self.service_start_time).total_seconds()
                    if time_since_startup < self.config.startup_delay_seconds:
                        # Still in startup delay period, wait
                        remaining_startup_delay = self.config.startup_delay_seconds - time_since_startup
                        wait_time = min(60, remaining_startup_delay)  # Check every minute during startup delay
                        logger.debug(f"⏳ Startup delay: {remaining_startup_delay:.0f} seconds remaining")
    ...
    async def _perform_sync(self):
        """Enhanced sync operation using comprehensive sync approach with proper delays"""
        if self.sync_in_progress:
        ...
        try:
            ...
            # Use comprehensive sync service if available
